...
public abstract class BaseExportHelper {
     ...
     * @param data the data
     */
    public abstract void export(ExportParams exportParams, List<?> data);
     ...
     */
    protected List<String> getHeadersLine(ExportParams exportParams){
        return exportParams.getColumns().stream().map(ExportColumn::getHeader).collect(Collectors.toList());
    }
     ...
     */
    protected List<List<String>> getDataLines(ExportParams exportParams, List<?> objectsToExport){
     ...
     */
    protected List<String> getDataLine(ExportParams exportParams, Object objectToExport){
        if(objectToExport == null || exportParams == null) return null;
        
        List<String> dataLine = new ArrayList<>();
        
        // iterate over column names
        for (ExportColumn column : exportParams.getColumns()) {
            String[] path = column.getColumn().split("\\.");
            log.trace("getDataLine() currentColumn={} , path={}", column.getColumn(), path); 
            
            // retrieve matching field recursively
            Object currentObject = objectToExport;
            for(String field : path) {
                if(currentObject != null) {
                    log.trace("getDataLine() currentObject={}, field={}", currentObject.getClass().getSimpleName(), field);
            ...
            if(currentObject != null) {
                log.trace("getDataLine() currentObject={}, value={}", currentObject.getClass().getSimpleName(), currentObject.toString());
    ...
    private Object getFieldValue(Object objectToExport, String fieldName) {
        if(objectToExport == null || fieldName == null) return null;
        
        log.trace("getFieldValue(objectToExport={}, fieldName={})", objectToExport.getClass().getSimpleName(), fieldName);
        ...
        log.trace("getFieldValue() result class={}, value={}", value!= null ? value.getClass().getSimpleName() : null, value);
        return value;
    }
